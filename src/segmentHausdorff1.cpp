...
void showSegment(vector<point> segment, point p1, point p2, float_t);
...
int main(){
  ...
  showSegment(compute2dSegment(p,q,XCoord,YCoord),p,q,slope);
 
  return 0;
}

void showSegment(vector<point> segment, point p1, point p2, float slope){
 ...
 for(int i=0; i<segment.size(); i++){
   ...
   float hausdorffDistance = computeHausdorff(segment[i], slope, p1); //Hausdorff distance
...
float computeHausdorff(point p, float slope, point p1){ //compute hausdorff distance
	...
	intercept = p1.coord[YCoord] - (slope * p1.coord[XCoord]);
	x = (p.coord[XCoord] + p.coord[YCoord] - intercept) / (slope + 1);
	y = p.coord[XCoord] + p.coord[YCoord] - x;
