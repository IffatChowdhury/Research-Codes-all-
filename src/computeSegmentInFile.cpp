...
int main(){
	...
	myfile.open ("output_star_red.txt");
	for(int i = x1; i <= x2; i++){
		for(int j = y1; j <= y2; j++){
			if (i == x2 && j == y2)
				break;
	...
	/*myfile.open ("output_projection_red.txt");	
	p.coord[XCoord] = x1;
	p.coord[YCoord] = y1;
	q.coord[XCoord] = x2;
	q.coord[YCoord] = y2;
...
vector<point> compute2dSegment(point p1,point p2, int dim1, int dim2){
  ...
  int noOfMovements = p2.coord[dim2] - p1.coord[dim2] + p2.coord[dim1] - p1.coord[dim1];
   myfile << noOfMovements << endl;
  if(p1.coord[dim1] == p2.coord[dim1] && p1.coord[dim2] == p2.coord[dim2]){
	...
	cout << endl << endl;*/
      ...
      myfile << currentPoint.coord[dim1] << " " << currentPoint.coord[dim2] << endl;
      segment.push_back(p1);
