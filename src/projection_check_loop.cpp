...
int main(){
	...
	x1_min = 0; x1_max = 0; x2_min = 25; x2_max = 55; y1 = 14; z1 = 2; y2 = 514; z2 = 502;
		...
		while(fold_iterate < (folds.size() - 1)){
			if (f == -1){ // xz planes are handled
				...
				if (check == -1){
					...
					cout << "fold : " << folds[fold_iterate] << endl;
					cout << p.coord[XCoord] << " " << p.coord[YCoord] << " " << p.coord[ZCoord] << endl;
					cout << q.coord[XCoord] << " " << q.coord[YCoord] << " " << q.coord[ZCoord] << endl;
