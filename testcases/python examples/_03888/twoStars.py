...
def main():
	...
	fileHandle = open('output_star_blue.txt', 'r')
	...
	while count != length:
		...
		for x in range(2, noOfMovements):
			...
			line = Line(Point(a + 1000, b + 1500), Point(c + 1000, d + 1500)) #650
			line.setWidth(2)
			line.setFill("blue")
			line.draw(win)
			#print a
	...
	win.postscript(file="twoStars.eps", colormode='color')

	# Convert from eps format to gif format using PIL
	from PIL import Image as NewImage
	img = NewImage.open("twoStars.eps")
	img.save("twoStars.gif", "gif")
	win.getMouse()
	win.close()
